...
static int certificates_provision(void)
{
	...
	if (exists) {
		mismatch = modem_key_mgmt_cmp(CONFIG_MQTT_TLS_SEC_TAG,
		...
		err = modem_key_mgmt_delete(CONFIG_MQTT_TLS_SEC_TAG, MODEM_KEY_MGMT_CRED_TYPE_CA_CHAIN);
		if (err) {
		      ...
		      const struct mqtt_evt *evt)
{
	...
	case MQTT_EVT_CONNACK:
		if (evt->result != 0) {
			LOG_ERR("MQTT connect failed: %d", evt->param.connack.return_code);
			break;
		}
...
static int client_init(struct mqtt_client *client)
{
	...
	username.utf8 = (uint8_t *)CONFIG_MQTT_BROKER_USERNAME;
	username.size = strlen(CONFIG_MQTT_BROKER_USERNAME);
